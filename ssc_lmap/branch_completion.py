...
class BranchCompletion:
    ...
    def predict_initial_cylinder(self, branch_pcd, cam_center, verbose=False, vis_check=False, return_axis=False):
        ...
        Returns:
            cylinder: open3d.geometry.TriangleMesh, predicted cylinder mesh
            axis: np.ndarray, axis of the cylinder
        """
    ...
    def get_cylinder_front_back_pts(self, cylinder_mesh, cylinder_axis, cam_center, vis_check=False):
        """
        A utility function to get the front and back points of the cylinder mesh
        from the given camera center view.
        
        Args:
            cylinder_mesh (open3d.geometry.TriangleMesh): The cylinder mesh.
            cylinder_axis (np.ndarray): Shape (3,), the axis of the cylinder in 3D space.
            cam_center (np.ndarray): Shape (3,), the camera center position in 3D space.
            vis_check (bool): Whether to visualize the front and back points.
        
        Returns:
            front_vertices_idx (np.ndarray): Indices of the front vertices of the cylinder mesh.
            back_vertices_idx (np.ndarray): Indices of the back vertices of the cylinder mesh.
        """
    ...
    def complete(self, branch_pcd, cam_center, voxel_size=0.003, match_dist_thresh=0.03, 
        ...
        and open3d as-rigid-as-possible deformation for shape registration.
        
        Pre-processing steps:
        1. Remove outliers from the branch point cloud using statistical outlier removal.
        2. Downsample the branch point cloud using voxel downsampling.
        3. Predict the initial cylinder shape using PCA and the average distance of points to the line.
        4. (Optional) detect the front and back points of the cylinder mesh based on the camera center view.
        
        For each iteration, we run the deformation registration process:
        1. Compute the nearest neighbors of the branch points in the cylinder mesh.
        2. Detect matching points based on the distance threshold.
        3. Deform the cylinder mesh using the ARAP deformation method.
        4. (Optional) smooth the branch mesh after deformation.
        
        The final deformed cylinder mesh is returned as the output.
        
        Args:
            branch_pcd (open3d.geometry.PointCloud): branch point cloud
            cam_center (np.ndarray): shape (3,), camera center position
            voxel_size (float): voxel size for downsampling
            match_dist_thresh (float): distance threshold for matching points
            max_iteration (int): maximum number of iterations for ARAP deformation
            arap_max_iter (int): maximum number of iterations for ARAP deformation
            reg_front_pts (bool): whether only register the front points of the cylinder
            smooth_branch (bool): whether to smooth the branch mesh after deformation
            verbose (bool): whether to print debug information
            vis_check (bool): whether to visualize the intermediate results using Open3D
        
        Returns:
            deform_cylinder_mesh (open3d.geometry.TriangleMesh): deformed cylinder mesh
        """
